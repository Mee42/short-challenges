...
fun chooseAction(board: Board, currentPlayer: Team): Triple<Point, Action, Direction?> {
    ...
    print("Do you want to rotate or move? Direction: ${piece.direction}: (r/m/cancel) ")
...
fun pickDirection(): Direction? {
    ...
    if (option == null) {
        println(
            "Direction must be one of the following: " + Direction.values().map { it.initials }.toString().drop(1).dropLast(
                1
            )
        )
